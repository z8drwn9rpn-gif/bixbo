...
s = cycle.read_text()
pattern = re.compile(r'export function SexForm\([\s\S]*?\n}\n\nexport function AddCustomInline', re.M)
new_form = r'''export function SexForm({ date, data, update, onDone, initialEntry }: { date: string; data: BixboData; update: UpdateFn; onDone: () => void; initialEntry?: SexEntry }) {
  const { t } = useI18n();
  ...
  return <div className="mx-auto flex w-full max-w-xl flex-col gap-4 pb-5">
    ...
    <section className="rounded-3xl bg-surface p-3 ring-1 ring-border/80">
      ...
      <Input type="time" value={time} onChange={(e) => setTime(e.target.value)} className="h-12 rounded-2xl bg-background/35 text-base" />
    </section>

    <section>
      <p className="mb-2 text-[11px] font-bold uppercase tracking-[0.12em] text-muted-foreground">{t("Type")}</p>
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
        {SEX_TYPES_DEFAULT.map((o) => <button key={o.value} type="button" onClick={() => setKind(o.value)} className={`min-h-[68px] rounded-3xl border px-2 py-2 text-xs font-semibold transition ${kind === o.value ? "border-primary bg-primary text-primary-foreground shadow-sm ring-1 ring-primary/30" : "border-border bg-surface text-foreground"}`}>{o.label}</button>)}
      </div>
    ...
    <section className="border-t border-border pt-4">
      ...
      <div className="grid grid-cols-3 gap-2">
        {([ ["yes", "✨", "Yes"], ["no", "○", "No"], ["unsure", "?", "Not sure"] ] as const).map(([value, icon, label]) => <button key={value} type="button" onClick={() => setOrgasm(orgasm === value ? undefined : value)} className={`flex h-12 items-center justify-center gap-2 rounded-2xl border text-xs font-semibold ${orgasm === value ? "border-primary bg-primary text-primary-foreground ring-1 ring-primary/30" : "border-border bg-surface"}`}><span className="text-base">{icon}</span><span>{t(label)}</span></button>)}
      </div>
    </section>
    ...
    <section className="border-t border-border pt-4">
      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
        {protectionOptions.map((value) => <button key={value} type="button" onClick={() => setProtection(protection === value ? "" : value)} className={`h-11 rounded-2xl border px-2 text-xs font-semibold ${protection === value ? "border-primary bg-primary/15 text-primary ring-1 ring-primary/30" : "border-border bg-surface"}`}>{t(value)}</button>)}
      </div>
    </section>
    ...
    <section className="border-t border-border pt-4">
      ...
      <div className="grid grid-cols-2 gap-2">
        {symptomOptions.map((value) => <button key={value} type="button" onClick={() => setSymptomsAfter((current) => toggleIn(current, value))} className={`min-h-10 rounded-2xl border px-2 text-[11px] font-medium ${symptomsAfter.includes(value) ? "border-primary bg-primary/15 text-primary ring-1 ring-primary/20" : "border-border bg-surface"}`}>{t(value)}</button>)}
      </div>
    </section>
...
export function AddCustomInline'''
if not pattern.search(s):
    raise SystemExit('SexForm block not found')
cycle.write_text(pattern.sub(new_form, s, count=1))

types = Path('src/lib/storage/types.ts')
t = types.read_text()
old = '''export interface SexEntry {\n  id: string;\n  time: string;\n  kind: SexKind;\n  feelingAfter?: string | string[];\n  painful?: PainfulWhen;\n  note?: string;\n}\n'''
new = '''export interface SexEntry {\n  id: string;\n  time: string;\n  kind: SexKind;\n  orgasm?: "yes" | "no" | "unsure";\n  protection?: string;\n  contraception?: string;\n  symptomsAfter?: string[];\n  feelingAfter?: string | string[];\n  painful?: PainfulWhen;\n  note?: string;\n}\n'''
if old not in t:
    raise SystemExit('SexEntry block not found')
