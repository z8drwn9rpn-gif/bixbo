      ...
      <div className="mb-2.5 flex items-center justify-between gap-3">
        ...
        <div className="relative flex items-center">
          <button
            ...
            className={`grid h-8 w-9 place-items-center rounded-full text-lg font-bold leading-none transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring ${
              editMode ? "bg-primary text-primary-foreground shadow-sm" : "text-muted-foreground hover:bg-tint hover:text-foreground"
            }`}
          >
            ⋯
          </button>
          {actionsOpen ? (
            <>
              <button type="button" aria-label={t("Close")} className="fixed inset-0 z-40 cursor-default" onClick={() => setActionsOpen(false)} />
              <div className="absolute right-0 top-9 z-50 min-w-[150px] rounded-2xl border border-border/70 bg-background p-1.5 shadow-xl">
                <button
                  type="button"
                  onClick={() => {
          ...
          {tags.map((tag, index) => {
            ...
            if (editMode) {
              return (
                <div
                  key={tag.key}
                  className="relative flex aspect-square min-w-0 select-none flex-col lg:mx-auto lg:w-full lg:max-w-[94px] xl:max-w-[100px] items-center justify-center gap-0.5 rounded-full bg-surface p-1.5 text-center shadow-sm ring-1 ring-border/80"
                >
                  <button
                    type="button"
