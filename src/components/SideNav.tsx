...
export function SideNav({ mascotSrc }: { mascotSrc: string; mascotFallbackSrc?: string }) {
  ...
  return (
    <aside className="fixed inset-y-0 left-0 z-40 hidden w-60 flex-col border-r border-[#cbd3a7]/85 bg-[#edf0c8]/96 px-3 py-5 backdrop-blur-xl lg:flex dark:border-border/70 dark:bg-[#303827]/96" style={{ boxShadow: "10px 0 30px -24px rgba(52,67,28,.55), inset -1px 0 0 rgba(255,255,255,.45), inset 0 18px 34px rgba(255,255,255,.16)" }}>
      ...
      <nav aria-label="Primary navigation" className="min-h-0 flex-1 overflow-y-auto">
        <ul className="flex flex-col gap-1.5">
          {navItems.map((navItem) => {
            ...
            if (navItem.action === "log") return <li key={navItem.id}><button type="button" onClick={openLog} className="flex min-h-[72px] w-full items-center gap-3 rounded-3xl px-3 py-2 text-left text-[15px] font-semibold text-[#415025] transition-[background-color,transform] hover:bg-white/22 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring active:scale-[0.985] dark:text-[#dfe9bd] dark:hover:bg-white/5"><NavArtwork id={navItem.id} size={56} className="-my-2 h-14 w-14 shrink-0 object-contain drop-shadow-[0_8px_9px_rgba(59,74,31,0.28)]" /><span>{t(label)}</span></button></li>;
            const to = navItem.to ?? "/";
            const active = navItem.id === "overview" ? pathname.startsWith("/insights") || pathname.startsWith("/patterns") : isActive(to);
            return <li key={navItem.id}><Link to={to as never} aria-current={active ? "page" : undefined} className={`relative flex min-h-[64px] items-center gap-3 rounded-3xl px-3 py-2 text-[15px] font-semibold transition-[background-color,transform] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring active:scale-[0.985] ${active ? "bg-white/28 text-[#3a4920] shadow-[inset_0_1px_0_rgba(255,255,255,.65),0_6px_16px_-14px_rgba(44,57,24,.5)] dark:bg-white/7 dark:text-[#e4edc7]" : "text-[#4b5930] hover:bg-white/18 hover:text-[#34411f] dark:text-[#bdc99e] dark:hover:bg-white/5 dark:hover:text-[#e4edc7]"}`}><NavArtwork id={navItem.id} size={46} className={`h-[46px] w-[46px] shrink-0 object-contain drop-shadow-[0_6px_7px_rgba(59,74,31,0.24)] transition-transform ${active ? "scale-[1.04]" : ""}`} /><span className="truncate">{t(label)}</span>{active ? <span aria-hidden="true" className="absolute left-1 top-1/2 h-7 w-1 -translate-y-1/2 rounded-full bg-current opacity-55" /> : null}</Link></li>;
          })}
        </ul>
      </nav>
    </aside>
