...
export function BottomNav() {
  ...
  return (
    <nav
      className="fixed inset-x-0 bottom-0 z-40 w-full border-t border-[#7f8d58]/55 bg-[#a8b27b] pb-[max(8px,env(safe-area-inset-bottom))] lg:hidden dark:border-border/70 dark:bg-[#303827]"
      style={{
        boxShadow:
          "0 -8px 22px -17px rgba(45,58,26,.55), inset 0 1px 0 rgba(255,255,255,.16)",
      }}
    >
        ...
        {items.map((item) => {
          ...
          return (
            <li key={item.id} className="flex min-w-0 flex-1 justify-center">
              <Link
                to={to as never}
                className={`flex min-h-[80px] w-full flex-col items-center justify-end gap-0 px-1 pb-0.5 text-[11px] font-semibold transition-transform focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring active:scale-[0.97] landscape:min-h-[68px] ${
                  active
                    ? "text-[#34431f] dark:text-[#e3edc4]"
