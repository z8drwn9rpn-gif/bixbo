...
export type AdminSectionDefinition = { id: string; label: string; order: number };
...
export function AdminPagePanel({
  ...
  return (
    <>
      ...
      <button type="button" onClick={onResetPage} className="w-full rounded-2xl bg-tint px-4 py-2.5 text-xs font-semibold text-muted-foreground ring-1 ring-border">{t("Reset current page customizations")}</button>
    </>
  );
}
