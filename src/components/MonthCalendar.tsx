...
export function MonthCalendar({ month, data, selected, onSelect, onSwipeMonth }: { month: Date; data: BixboData; selected: string; onSelect: (k: string) => void; onSwipeMonth?: (delta: -1 | 1) => void; }) {
  ...
  const predictedKeys = useMemo(() => {
    ...
    for (const cell of cells) {
      ...
      if (!hasLoggedPeriod && predicted.some((p) => isDateInRange(cell.key, p.start, p.end))) result.add(cell.key);
    }
    return result;
  }, [cells, data.cycle, data.dayLogs, hidePredictions]);

  const cellMeta = useMemo(() => {
    const meta = new Map<string, { periodColor: string | null; pAvg: number | null; predictedPeriod: boolean; icons: string[] }>();
    for (const cell of cells) {
      ...
      if (!cycleTrackingHidden && data.cycle.lastPeriodStart && data.cycle.lastPeriodEnd && isDateInRange(cell.key, data.cycle.lastPeriodStart, data.cycle.lastPeriodEnd)) actualPeriodColor = "var(--period-medium)";
      meta.set(cell.key, {
  ...
  const weekLayouts = useMemo(() => {
    return weeks.map((week) => {
      ...
      const segments = data.events.filter((event) => event.startDate <= weekEnd && event.endDate >= weekStart).map((event) => {
        const startIdx = Math.max(0, week.findIndex((cell) => cell.key >= event.startDate && cell.key <= event.endDate));
        let endIdx = startIdx;
        for (let i = week.length - 1; i >= 0; i--) {
          if (week[i].key >= event.startDate && week[i].key <= event.endDate) { endIdx = i; break; }
        }
        return { event, startIdx, endIdx, showTitle: event.startDate >= weekStart || startIdx === 0 };
      });
      ...
      for (const segment of segments) {
        for (let rowIndex = 0; rowIndex < 2; rowIndex++) {
          let available = true;
          for (let i = segment.startIdx; i <= segment.endIdx; i++) if (usedByCell[rowIndex][i]) { available = false; break; }
          if (available) {
            rows[rowIndex].push(segment);
            for (let i = segment.startIdx; i <= segment.endIdx; i++) usedByCell[rowIndex][i] = true;
            break;
          }
        }
      }
  ...
  const onTouchEnd = (e: React.TouchEvent) => {
    ...
    if (Math.abs(dx) > 80 && Math.abs(dx) > Math.abs(dy) * 1.8) onSwipeMonth(dx > 0 ? -1 : 1);
  ...
  return (
    <div className="px-1 landscape:px-2 lg:px-2" onTouchStart={onTouchStart} onTouchEnd={onTouchEnd}>
      <div className="grid grid-cols-7 gap-0.5 pb-1 text-center text-[11px] font-semibold text-muted-foreground landscape:pb-0 landscape:text-[10px] lg:pb-2 lg:text-sm xl:text-[15px]">
        {WEEKDAYS.map((d) => <div key={d.short}><span className="lg:hidden">{t(d.short)}</span><span className="hidden lg:inline">{t(d.desktop)}</span></div>)}
      </div>
        ...
        {weeks.map((week, wi) => {
          ...
          return (
            <div key={wi} className="grid grid-cols-7 gap-x-0.5 gap-y-0.5 lg:gap-x-1.5 lg:gap-y-1 xl:gap-x-2 xl:gap-y-1.5">
                ...
                return (
                  <button key={ci}
                    ...
                    onPointerMove={(event) => {
                      ...
                      if (!start) return;
                      if (Math.abs(event.clientX - start.x) > 8 || Math.abs(event.clientY - start.y) > 8) { dayPointerMoved.current = true; clearLong(); }
                    }}
                    onPointerUp={() => {
                      clearLong();
                    ...
                    onPointerLeave={() => { dayPointerMoved.current = true; dayPointerStart.current = null; clearLong(); }}
                    onPointerCancel={() => { dayPointerMoved.current = true; dayPointerStart.current = null; clearLong(); }}
                    onContextMenu={(e) => e.preventDefault()}
                    className={`flex select-none flex-col items-stretch rounded-xl text-left transition ${inMonth ? "" : "opacity-30"} ${isSel ? "bg-primary/5" : ""}`}>
                    <div className="flex min-h-[62px] flex-col items-center justify-start pt-1 landscape:min-h-[44px] landscape:pt-0 lg:min-h-[84px] lg:pt-2 xl:min-h-[90px]">
                      <div className="relative flex h-9 w-9 shrink-0 items-center justify-center rounded-full lg:h-[52px] lg:w-[52px] xl:h-[56px] xl:w-[56px]">
                        {pAvg != null && <span aria-hidden className="pointer-events-none absolute inset-0 rounded-full" style={{ boxShadow: `0 0 0 4px ${calendarPainColor(pAvg)}` }} />}
                        {predictedPeriod && <span aria-hidden className="pointer-events-none absolute inset-[4px] rounded-full lg:inset-[5px]" style={{ boxShadow: `inset 0 0 0 2.5px var(--period-medium)` }} />}
                        <div className={`relative z-[1] flex h-7 w-7 items-center justify-center rounded-full lg:h-10 lg:w-10 xl:h-11 xl:w-11 ${isSel ? "ring-2 ring-primary ring-offset-1 ring-offset-background" : ""}`} style={{ background: periodColor ?? "transparent" }}>
                          <span className={`text-sm leading-none lg:text-base xl:text-[17px] ${periodColor ? "font-semibold text-white" : "text-foreground"}`}>{date.getDate()}</span>
                        </div>
                      </div>
                      <div className="mt-0.5 flex h-4 items-center justify-center leading-none lg:mt-1 lg:h-5">
                        {icons.includes("❤️") ? <Ico e="❤️" size={14} className="h-[14px] w-[14px] lg:h-[17px] lg:w-[17px] xl:h-[18px] xl:w-[18px]" /> : <span aria-hidden className="block h-[14px] w-[14px] lg:h-[17px] lg:w-[17px]" />}
                      </div>
                    </div>
                  </button>
                );
                ...
                <div key={`r-${ri}`} className="col-span-7 grid grid-cols-7 gap-0.5">
                  {new Array(7).fill(null).map((_, ci) => {
                    ...
                    if (seg) {
                      ...
                      return <div key={ci} className={`h-3 truncate px-1 text-[10px] font-medium leading-3 text-primary-foreground lg:h-3.5 lg:text-[10px] lg:leading-[14px] xl:h-4 xl:text-[11px] xl:leading-4 ${isStart ? "rounded-l" : ""} ${isEnd ? "rounded-r" : ""}`} style={{ gridColumn: `span ${span} / span ${span}`, background: seg.event.color ?? "var(--primary)" }}>{seg.event.title}</div>;
                    }
              ...
              {overflowByCell.some((n) => n > 0) && <div className="col-span-7 grid grid-cols-7 gap-0.5">{overflowByCell.map((n, ci) => <div key={ci} className="h-[10px] text-center text-[10px] leading-[10px] text-muted-foreground lg:h-3 lg:text-[10px] lg:leading-3">{n > 0 ? `+${n}` : ""}</div>)}</div>}
            </div>
          );
        })}
      </div>
      ...
      {peek && typeof document !== "undefined" ? createPortal((() => {
        ...
        return (
          <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/40 px-5 py-[max(16px,env(safe-area-inset-top))] backdrop-blur-[1px]" onClick={() => setPeek(null)}>
            <section role="dialog" aria-modal="true" aria-label={t("Day summary")} className="flex max-h-[calc(100dvh-32px)] w-full max-w-[350px] flex-col overflow-hidden rounded-[30px] border border-border/70 bg-background shadow-[0_24px_70px_-30px_rgba(24,31,17,.55),0_6px_20px_-12px_rgba(24,31,17,.35)]" onClick={(event) => event.stopPropagation()}>
              <div className="shrink-0 px-5 pb-3 pt-5">
                <div className="flex items-start justify-between gap-4">
                  <div className="min-w-0">
                    <p className="text-[10px] font-bold uppercase tracking-[0.23em] text-muted-foreground">{t("Day summary")}</p>
                    <h3 className="mt-1.5 text-[21px] font-black tracking-[-0.035em] text-foreground">{d.toLocaleDateString("en-GB", { weekday: "long", day: "numeric", month: "long" })}</h3>
                  </div>
                ...
                <button type="button" onClick={() => { onSelect(peek); setPeek(null); }} className="mt-3 flex w-full items-center gap-3 rounded-[19px] border border-border/55 bg-tint/45 px-3 py-2.5 text-left transition active:scale-[0.99]">
                  ...
                  <span className="min-w-0 flex-1"><span className="block text-[13px] font-bold text-foreground">{entryCount} {entryCount === 1 ? "entry" : "entries"} · {summary.length} {summary.length === 1 ? "log" : "logs"}</span><span className="mt-0.5 block text-[11px] text-muted-foreground">{t("Tap to open this day")}</span></span>
                  <span aria-hidden className="text-lg text-muted-foreground">›</span>
                </button>
              </div>

              <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-4 pb-[max(16px,env(safe-area-inset-bottom))] touch-pan-y">
                {summary.length === 0 ? <div className="rounded-[20px] border border-border/55 bg-tint/30 px-4 py-5 text-center text-sm text-muted-foreground">{t("Nothing logged on this day.")}</div> : (
                  <div className="overflow-hidden rounded-[22px] border border-border/55 bg-background/75">
                    {summary.map((row, index) => <button key={`${row.label}-${index}`} type="button" onClick={() => { onSelect(peek); setPeek(null); }} className={`grid w-full grid-cols-[34px_minmax(0,1fr)_auto_14px] items-center gap-2 px-3 py-2.5 text-left transition hover:bg-tint/35 active:bg-tint/55 ${index ? "border-t border-border/45" : ""}`}>
                      <span className="grid h-8 w-8 place-items-center text-[18px] leading-none"><Ico e={row.icon} size={18} /></span>
                      <span className="min-w-0"><span className="block truncate text-[12.5px] font-bold leading-tight text-foreground">{row.label}</span>{row.meta ? <span className="mt-0.5 block truncate text-[10.5px] leading-tight text-muted-foreground">{row.meta}</span> : null}</span>
