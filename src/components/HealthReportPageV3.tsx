...
function doseSummary(m:Med,days:RDay[],medLog:MedLog,now:Date) {
...
function packTimeline(rows:RDay[],meds:Med[],medLog:MedLog){const pages:RDay[][]=[];let page:RDay[]=[];let budget=0;for(const d of rows){const text=[facts(d).join(" "),takenText(d,meds,medLog),extraText(d),tensText(d),contextText(d)].join(" ");const cost=Math.max(1,Math.ceil(text.length/170));if(page.length&&(budget+cost>30||page.length>=19)){pages.push(page);page=[];budget=0;}page.push(d);budget+=cost;}if(page.length)pages.push(page);if(!pages.length)pages.push([]);return pages;}
...
function Report({days,meds,medLog,range,locale}:{days:RDay[];meds:Med[];medLog:MedLog;range:string;locale:string}) {
  ...
  const scheduled=meds.filter(m=>!m.asNeeded&&(m.times?.length??0)>0),prn=meds.filter(m=>m.asNeeded),detailPages=packTimeline([...logged].reverse(),meds,medLog),total=3+detailPages.length,now=new Date();
...
export function HealthReportPageV3() {
  ...
  const range=`${longDate(start,locale)} – ${longDate(end,locale)} · ${days.length}-day report`,report=<Report days={days} meds={view.meds} medLog={view.medLog} range={range} locale={locale}/>;
