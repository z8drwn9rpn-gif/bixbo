...
import { parseRecipeNote } from "./RecipesView";
...
function sectionsToText(sections: RecipeSection[]): string {
...
function textToSections(value: string): RecipeSection[] {
...
function blankCoffee(): CoffeeDraft {
  return {
    title: "",
    ingredients: "",
    method: "",
...
function draftFromCoffee(note: Note): CoffeeDraft {
  const recipe = note.recipe;
  return {
    id: note.id,
    title: note.title,
    ingredients: sectionsToText(recipe?.ingredientSections ?? []),
  ...
  return (
    <svg viewBox="0 0 108 82" className={className} fill="none" aria-hidden="true">
      <g stroke="currentColor" strokeLinecap="round" strokeLinejoin="round">
        <ellipse cx="48" cy="29" rx="34" ry="14" strokeWidth="1.6" />
        <ellipse cx="48" cy="29" rx="28" ry="10" strokeWidth="1.1" opacity=".65" />
        <path d="M15 29c1 21 7 38 33 41 25-3 31-20 33-41" strokeWidth="1.7" />
        <path d="M80 34c14-4 22 1 20 11-2 10-11 15-22 12" strokeWidth="1.7" />
        <path d="M84 39c8-2 12 1 11 6-1 5-6 8-12 7" strokeWidth="1.2" />
        <path d="M31 27c5-7 12-7 17 0 5-7 12-7 17 0-4 2-7 5-9 9-2-3-5-5-8-7-3 2-6 4-8 7-2-4-5-7-9-9Z" strokeWidth="1.1" opacity=".75" />
        <path d="M27 44c3 8 8 13 15 16M69 43c-3 9-8 14-15 17M23 50c2 4 5 8 9 11" strokeWidth=".9" opacity=".38" />
        <path d="M17 72h64" strokeWidth="1" opacity=".45" />
      </g>
    </svg>
  );
}
...
export function CoffeeView({ data, update }: { data: BixboData; update: UpdateFn }) {
  ...
  const [preview, setPreview] = useState<ReturnType<typeof parseRecipeNote>>([]);
  ...
  const filtered = useMemo(() => {
    ...
    return coffee.filter((note) => !needle || coffeeSearchText(note).includes(needle));
  }, [coffee, query]);
  ...
  const saveCoffee = () => {
    ...
    const ingredientSections = textToSections(editor.ingredients);
    ...
    const recipe: RecipeData = {
      ...
      status: ingredientSections.length && method.length ? "ready" : "draft",
    };
  ...
  const deleteAllCoffee = () => {
    if (!coffee.length || !window.confirm(t("Delete all coffee? This cannot be undone."))) return;
    update((currentData) => ({
      ...currentData,
      notebook: currentData.notebook.filter((saved) => saved.kind !== "coffee"),
    }));
    setQuery("");
    setIndex(0);
  };
  ...
  const importCoffee = () => {
    ...
    const imported: Note[] = preview
      ...
      .map((item, offset) => ({
        ...
        recipe: { ...item.recipe, category: "other" as const, sourceNoteId: source.id },
      }));
    ...
    return (
      <section className="space-y-4 pb-4">
        <div className="flex items-center justify-between gap-3">
          ...
          <Button className="rounded-full px-6" onClick={saveCoffee} disabled={!editor.title.trim()}>{t("Save")}</Button>
        </div>
        ...
        <div className="relative mx-auto w-full max-w-[760px] pb-2 pr-2">
          ...
          <div className="relative overflow-hidden rounded-[28px] border border-[#D4CCB8] bg-[#FBF7EC] px-5 pb-5 pt-4 text-[#303126] shadow-[0_10px_28px_rgba(70,62,43,0.12)] sm:px-7">
            <CoffeeCupSketch className="absolute right-3 top-2 h-[80px] w-[104px] text-[#6E7447] opacity-85" />
            <div className="pr-28">
            ...
            <div className="mt-4 grid grid-cols-[0.82fr_0.82fr_1.08fr_1.28fr] border-y border-[#CFC5AD] py-3 text-[#45483A]">
              <label className="min-w-0 border-r border-[#D8D0BC] pr-2"><span className="flex items-center gap-0.5 whitespace-nowrap text-[8px] font-bold uppercase tracking-[0.06em] text-[#7B8063]"><MetaIcon kind="time" />{t("Prep")}</span><Input inputMode="numeric" placeholder="—" value={editor.prepMinutes} onChange={(event) => setEditor({ ...editor, prepMinutes: event.target.value })} className="mt-1 h-8 rounded-none border-0 bg-transparent px-0 text-sm font-semibold shadow-none focus-visible:ring-0" /></label>
              <label className="min-w-0 border-r border-[#D8D0BC] px-2"><span className="flex items-center gap-0.5 whitespace-nowrap text-[8px] font-bold uppercase tracking-[0.06em] text-[#7B8063]"><MetaIcon kind="time" />{t("Cook")}</span><div className="flex items-center gap-1"><Input inputMode="numeric" placeholder="—" value={editor.cookMinutes} onChange={(event) => setEditor({ ...editor, cookMinutes: event.target.value })} className="mt-1 h-8 min-w-0 rounded-none border-0 bg-transparent px-0 text-sm font-semibold shadow-none focus-visible:ring-0" /><Input inputMode="numeric" placeholder="°C" value={editor.temperatureC} onChange={(event) => setEditor({ ...editor, temperatureC: event.target.value })} className="mt-1 h-8 w-12 rounded-none border-0 bg-transparent px-0 text-[11px] font-semibold shadow-none focus-visible:ring-0" /></div></label>
              <label className="min-w-0 border-r border-[#D8D0BC] px-2"><span className="flex items-center gap-0.5 whitespace-nowrap text-[8px] font-bold uppercase tracking-[0.06em] text-[#7B8063]"><MetaIcon kind="people" />{t("Portions")}</span><Input placeholder="—" value={editor.portions} onChange={(event) => setEditor({ ...editor, portions: event.target.value })} className="mt-1 h-8 rounded-none border-0 bg-transparent px-0 text-sm font-semibold shadow-none focus-visible:ring-0" /></label>
              <div className="min-w-0 pl-2"><span className="flex items-center gap-0.5 whitespace-nowrap text-[8px] font-bold uppercase tracking-[0.06em] text-[#7B8063]"><MetaIcon kind="tag" />{t("Category")}</span><p className="mt-2 text-[11px] font-bold">{t("Coffee")}</p></div>
            </div>

            <div className="mt-5 grid grid-cols-[0.9fr_1.1fr] divide-x divide-[#CFC5AD]">
              <div className="min-w-0 pr-4"><p className="mb-2 text-sm font-bold">{t("Ingredients")}</p><Textarea rows={14} value={editor.ingredients} onChange={(event) => setEditor({ ...editor, ingredients: event.target.value })} placeholder={t("One ingredient per line") } className="min-h-[340px] resize-y border-[#D8D0BC] bg-white/20 px-2 py-2 text-[12px] leading-6 shadow-none focus-visible:ring-[#87944D]" /></div>
              <div className="min-w-0 pl-4"><p className="mb-2 text-sm font-bold">{t("Method")}</p><Textarea rows={14} value={editor.method} onChange={(event) => setEditor({ ...editor, method: event.target.value })} placeholder={t("One step per line")} className="min-h-[340px] resize-y border-[#D8D0BC] bg-white/20 px-2 py-2 text-[12px] leading-6 shadow-none focus-visible:ring-[#87944D]" /></div>
            </div>
  ...
  return (
    <section className="space-y-4 pb-4">
      <div className="flex justify-end -mb-1"><button type="button" onClick={() => { if (searchOpen) setQuery(""); setSearchOpen(!searchOpen); }} aria-label={t("Search coffee")} aria-expanded={searchOpen} className={`inline-flex min-h-10 items-center gap-2 rounded-full border px-3 text-xs font-semibold shadow-sm transition ${searchOpen ? "border-primary bg-primary text-primary-foreground" : "border-[#D4CCB8] bg-[#FBF7EC] text-[#555A3E]"}`}><Search className="h-4 w-4" />{t("Search")}</button></div>

      {searchOpen ? <div className="relative rounded-[22px] border border-border/60 bg-surface/70 p-3 shadow-sm"><Search className="pointer-events-none absolute left-6 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={t("Search coffee…")} className="h-11 rounded-2xl border-border/70 bg-background pl-10 pr-11 shadow-none" />{query ? <button type="button" onClick={() => setQuery("")} aria-label={t("Clear search")} className="absolute right-4 top-1/2 grid h-9 w-9 -translate-y-1/2 place-items-center rounded-xl text-muted-foreground hover:bg-tint"><X className="h-4 w-4" /></button> : null}</div> : null}
      ...
      {!importOpen && current?.recipe ? <div className="relative mx-auto w-full max-w-[760px] touch-pan-y pb-5 pr-5" onTouchStart={(event) => { const touch = event.touches[0]; touchStart.current = { x: touch.clientX, y: touch.clientY }; }} onTouchEnd={(event) => { const start = touchStart.current; touchStart.current = null; if (!start) return; const touch = event.changedTouches[0]; const dx = touch.clientX - start.x; const dy = touch.clientY - start.y; if (Math.abs(dx) < 55 || Math.abs(dx) < Math.abs(dy)) return; move(dx < 0 ? 1 : -1); }}>
        ...
        <article className="relative z-10 overflow-hidden rounded-[28px] border border-[#D4CCB8] bg-[#FBF7EC] px-5 pb-5 pt-4 text-[#303126] shadow-[0_12px_30px_rgba(70,62,43,0.14)] sm:px-7"><CoffeeCupSketch className="absolute right-3 top-2 h-[82px] w-[108px] text-[#6E7447] opacity-85" /><div className="min-h-[76px] pr-28"><h2 className="text-[31px] leading-[1.05] text-[#2E3025] sm:text-[36px]" style={{ fontFamily: '"Snell Roundhand", "Segoe Script", "Bradley Hand", cursive' }}>{current.title}</h2></div>
          <div className="mt-2 grid grid-cols-[0.82fr_0.82fr_1.08fr_1.28fr] border-y border-[#CFC5AD] py-3 text-[#45483A]"><div className="min-w-0 border-r border-[#D8D0BC] pr-2"><p className="flex items-center gap-0.5 whitespace-nowrap text-[8px] font-bold uppercase tracking-[0.06em] text-[#7B8063]"><MetaIcon kind="time" />{t("Prep")}</p><p className="mt-1 text-[12px] font-bold">{current.recipe.prepMinutes != null ? `${current.recipe.prepMinutes} min` : "—"}</p></div><div className="min-w-0 border-r border-[#D8D0BC] px-2"><p className="flex items-center gap-0.5 whitespace-nowrap text-[8px] font-bold uppercase tracking-[0.06em] text-[#7B8063]"><MetaIcon kind="time" />{t("Cook")}</p><p className="mt-1 text-[12px] font-bold">{current.recipe.cookMinutes != null ? `${current.recipe.cookMinutes} min` : "—"}{current.recipe.temperatureC != null ? <span className="block text-[9px] font-bold text-[#777B61]">{current.recipe.temperatureC} °C</span> : null}</p></div><div className="min-w-0 border-r border-[#D8D0BC] px-2"><p className="flex items-center gap-0.5 whitespace-nowrap text-[8px] font-bold uppercase tracking-[0.06em] text-[#7B8063]"><MetaIcon kind="people" />{t("Portions")}</p><p className="mt-1 truncate text-[12px] font-bold">{current.recipe.portions || "—"}</p></div><div className="min-w-0 pl-2"><p className="flex items-center gap-0.5 whitespace-nowrap text-[8px] font-bold uppercase tracking-[0.06em] text-[#7B8063]"><MetaIcon kind="tag" />{t("Category")}</p><p className="mt-1 truncate text-[11px] font-bold">{t("Coffee")}</p></div></div>
      ...
      {!importOpen ? <div className="flex flex-col items-center gap-2 pt-1"><Button className="h-14 w-full max-w-[320px] rounded-full text-base font-semibold shadow-sm" onClick={() => setEditor(blankCoffee())}><Plus className="h-5 w-5" />{t("New coffee")}</Button><button type="button" onClick={() => { setImportOpen(true); setPreview([]); }} className="min-h-10 rounded-full px-4 text-xs font-semibold text-muted-foreground hover:bg-tint">{t("Import from Notes")}</button>{coffee.length > 0 ? <button type="button" onClick={deleteAllCoffee} className="min-h-10 rounded-full px-4 text-xs font-semibold text-destructive hover:bg-destructive/5">{t("Delete all coffee")}</button> : null}</div> : null}
    </section>
  );
}
