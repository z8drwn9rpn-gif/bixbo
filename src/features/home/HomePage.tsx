...
export function HomePage() {
  ...
  return <AppShell
    big
    title={<div className="flex flex-col leading-tight"><span className="font-sans text-[37px] font-black tracking-[-0.05em] leading-[0.92] sm:text-[41px]" style={{ textShadow: "0 1px 0 rgba(255,255,255,.78), 0 2px 0 rgba(91,108,54,.24), 0 3px 1px rgba(68,83,39,.18), 0 6px 10px rgba(49,61,31,.14)" }}>BIXBO</span><span className="mt-1 inline-flex items-center gap-1 text-xs font-normal tracking-normal text-muted-foreground">{t("Hi")}, {view.settings.userName?.trim() || t("there")} <Ico e="❤️" size={12} /></span></div>}
    ...
    <div className="lg:mx-auto lg:grid lg:w-full lg:max-w-[1480px] lg:grid-cols-[minmax(0,1.62fr)_minmax(340px,0.95fr)] lg:items-start lg:gap-4 lg:px-0 xl:grid-cols-[minmax(0,1.72fr)_minmax(380px,1fr)] xl:gap-5">
      <div className="flex min-w-0 flex-col">
        <div style={{ order: layoutOrder(view, "home", "calendar", 10) }}>
          <div className="px-5 pt-1 lg:px-1"><div className="flex items-center justify-between"><button type="button" onClick={() => moveCalendarMonth(-1)} aria-label={t("Previous month")} className="rounded-full p-1.5 hover:bg-tint"><ChevronLeft className="h-5 w-5" /></button><h2 className="font-sans text-[29px] font-black tracking-[-0.04em] leading-none text-foreground sm:text-[31px] lg:text-[33px]" style={{ textShadow: "0 1px 0 rgba(255,255,255,.84), 0 2px 0 rgba(95,112,51,.20), 0 3px 1px rgba(68,83,39,.13), 0 5px 8px rgba(49,61,31,.10)" }} suppressHydrationWarning>{hydrated ? monthLabel(monthAnchor) : ""}</h2><button type="button" onClick={() => moveCalendarMonth(1)} aria-label={t("Next month")} className="rounded-full p-1.5 hover:bg-tint"><ChevronRight className="h-5 w-5" /></button></div></div>
