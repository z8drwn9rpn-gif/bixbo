      ...
      >
        <SheetHeader className="h-14 shrink-0 flex-row items-end justify-between gap-0 border-b border-border px-5 pb-2 pt-0">
          <button
            type="button"
            onClick={close}
            className="flex min-w-[76px] items-center gap-1 text-sm text-muted-foreground"
          >
            <ChevronLeft className="h-3.5 w-3.5 shrink-0" />
            {t("Overview")}
          </button>
          <SheetTitle className="font-serif text-lg">{t("Pain")}</SheetTitle>
          <button type="button" onClick={close} aria-label={t("Close")} className="rounded-full p-1 hover:bg-tint">
            <X className="h-5 w-5" />
          </button>
        </SheetHeader>
