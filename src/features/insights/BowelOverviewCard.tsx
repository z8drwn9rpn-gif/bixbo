...
export function BowelOverviewCard({
  ...
  return (
    <section data-bowel-overview-card="true" className="rounded-3xl bg-surface p-4 shadow-sm ring-1 ring-border/80">
      <div className="flex items-start justify-between gap-3">
        <div className="flex min-w-0 items-center gap-2.5">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-0.5 whitespace-nowrap text-[11px] text-muted-foreground">{t("Distribution of logged bowel types")}</p>
          </div>
        </div>
        <button
          type="button"
          ...
          className="grid h-8 w-8 shrink-0 place-items-center rounded-full text-muted-foreground transition hover:bg-tint hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
        >
          <InfoIcon />
        </button>
      </div>
      ...
      {showInfo ? (
        <div className="mt-2.5 rounded-xl bg-tint/55 px-3 py-2.5 text-[10px] leading-relaxed text-muted-foreground ring-1 ring-border/50">
      ...
      <div className="relative mt-3">
        <div className="grid h-[150px] grid-cols-8 items-end gap-1.5 border-b border-border/70 px-1 pb-0">
          {typeMeta.map((item) => {
            const pct = totalEntries ? Math.round((item.count / totalEntries) * 100) : 0;
            const selected = activeType === item.type;
            const height = item.count ? Math.max(14, (item.count / maxCount) * 96) : 4;
            return (
              <div key={item.type} className="flex h-full min-w-0 flex-col items-center justify-end">
                <span className="mb-1 whitespace-nowrap rounded-full bg-background/90 px-1 py-0.5 text-[8px] font-semibold tabular-nums text-foreground ring-1 ring-border/70">
                  {item.count} ({pct}%)
                </span>
                {item.count ? (
                  <button
                    ...
                    className={`w-full max-w-[38px] rounded-t-[6px] transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring ${selected ? "ring-2 ring-foreground/75" : ""}`}
                    style={{ height, background: item.color }}
                  />
                ) : (
                  <div className="w-full max-w-[38px] rounded-t-full" style={{ height, background: item.color }} />
                )}
                <span className="mt-1.5 text-[10px] font-semibold tabular-nums" style={{ color: item.tooltipColor }}>T{item.type}</span>
                <span className="text-[9px] tabular-nums text-muted-foreground">{item.count}</span>
              </div>
            );
          })}
        </div>
      ...
      <div className="mt-3 grid grid-cols-3 divide-x divide-border/70 rounded-xl bg-background/55 py-2 ring-1 ring-border/50">
        <div className="flex min-w-0 items-center gap-1 px-1.5">
          <span className="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-primary/10 text-primary"><SummaryIcon kind="entries" /></span>
          <div className="min-w-0"><p className="whitespace-nowrap text-[11px] font-bold tabular-nums text-foreground">{totalEntries} {totalEntries === 1 ? t("entry") : t("entries")}</p><p className="whitespace-nowrap text-[8px] text-muted-foreground">this {periodWord}</p></div>
        </div>
        <div className="flex min-w-0 items-center gap-1 px-1.5">
          <span className="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-primary/10 text-primary"><SummaryIcon kind="common" /></span>
          <div className="min-w-0"><p className="whitespace-nowrap text-[8px] text-muted-foreground">{t("Most common")}</p><p className="whitespace-nowrap text-[11px] font-bold text-foreground">{mostCommon ? `Type ${mostCommon.type}` : "—"}</p></div>
        </div>
        <div className="flex min-w-0 items-center gap-1 px-1.5">
          <span className="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-primary/10 text-primary"><SummaryIcon kind="none" /></span>
          <div className="min-w-0"><p className="whitespace-nowrap text-[7.5px] text-muted-foreground">{t("No bowel movements")}</p><p className="whitespace-nowrap text-[11px] font-bold tabular-nums text-foreground">{noBowelMovementCount} {noBowelMovementCount === 1 ? t("day") : t("days")}</p></div>
        </div>
      </div>

      <div className="mt-2.5 rounded-xl bg-primary/[0.07] px-3 py-2.5 ring-1 ring-primary/10">
        <div className="flex items-center gap-1.5 text-primary">
          <span className="grid h-6 w-6 place-items-center rounded-full bg-primary/10"><SummaryIcon kind="common" /></span>
          <p className="text-[10px] font-bold uppercase tracking-[0.08em]">{t("Quick insights")}</p>
        </div>
        <div className="mt-1.5 space-y-1 pl-0.5 text-[10px] text-foreground">
      ...
      <div className="mt-3 flex gap-1.5 overflow-x-auto pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden" role="group" aria-label={t("Filter bowel entries by Bristol type")}>
        <button
          type="button"
          onClick={() => setFilterType(null)}
          aria-pressed={filterType == null}
          className={`shrink-0 rounded-full px-3 py-1.5 text-[10px] font-bold transition ${filterType == null ? "bg-primary text-primary-foreground shadow-sm" : "bg-tint text-muted-foreground hover:text-foreground"}`}
        >
          {t("All")}
        </button>
        {typeMeta.map((item) => {
          ...
          return (
            <button
              ...
              aria-pressed={selected}
              className="shrink-0 rounded-full px-3 py-1.5 text-[10px] font-bold transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
              style={{
      ...
      <div className="mt-3">
        <div className="flex items-center justify-between gap-3">
          <h4 className="text-xs font-bold text-foreground">{t("Latest entries")}</h4>
          {filteredEntries.length > 2 ? (
            <button type="button" onClick={() => setShowAll((value) => !value)} className="text-[10px] font-semibold text-muted-foreground hover:text-foreground">
              {showAll ? t("Show less") : t("See all")} <span aria-hidden="true">›</span>
            </button>
          ) : null}
        </div>

        <div className="mt-1.5 overflow-hidden rounded-xl bg-background/45 ring-1 ring-border/60">
          {visibleEntries.length ? visibleEntries.map((entry, index) => (
            <div key={entry.id} className={`flex items-center gap-2.5 px-3 py-2 ${index ? "border-t border-border/60" : ""}`}>
              <div className="w-[48px] shrink-0">
                <p className="text-[10px] font-bold tabular-nums text-foreground">{fromKey(entry.key).toLocaleDateString("en-GB", { day: "numeric", month: "short" })}</p>
                <p className="text-[9px] tabular-nums text-muted-foreground">{entry.time || "—"}</p>
              </div>
              <span
                className="grid h-6 min-w-10 shrink-0 place-items-center rounded-full px-2 text-[10px] font-bold"
                style={{ color: entry.color, background: `color-mix(in srgb, ${entry.color} 12%, var(--surface))` }}
              >
                T{entry.type}
              </span>
              <p className="min-w-0 flex-1 truncate text-[10px] font-semibold text-foreground">{entry.description}</p>
              <span className="text-base leading-none text-muted-foreground" aria-hidden="true">›</span>
            </div>
          )) : (
            <p className="px-4 py-4 text-center text-[10px] text-muted-foreground">{t("No bowel entries in this period")}</p>
          )}
        </div>
      </div>
    </section>
