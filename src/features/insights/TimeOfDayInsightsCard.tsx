...
export function TimeOfDayInsightsCard({ data, days, period, anchor, onPeriodChange, onPeriodShift }: {
  ...
  return (
    <section className="rounded-3xl bg-surface p-5 shadow-sm ring-1 ring-border/80">
      <p className="text-sm uppercase tracking-[0.08em] text-muted-foreground" style={{ fontWeight: 700 }}>{t("Time of Day Pattern")}</p>
      ...
      {!total ? <p className="mt-4 text-sm text-muted-foreground">{t("Not enough data yet")}</p> : (
        <>
          <div className="mt-4 flex gap-5 text-sm text-foreground">
            <span className="flex items-center gap-2"><span className="h-3 w-3 rounded-full" style={{ background: TETANY_COLOR }} /> Tetany ({tetanyTotal})</span>
            <span className="flex items-center gap-2"><span className="h-3 w-3 rounded-full" style={{ background: PANIC_COLOR }} /> Panic ({panicTotal})</span>
          </div>

          <div className="mt-4 rounded-[26px] bg-background/45 px-3 py-4 ring-1 ring-border/45">
            <p className="text-[11px] text-muted-foreground">Number of events</p>
            <div className="mt-2 flex gap-2">
              <div className="flex h-[190px] w-5 flex-col justify-between text-right text-[11px] text-muted-foreground">{[3, 2, 1, 0].map((value) => <span key={value} className="leading-none">{value}</span>)}</div>
              <div className="relative min-w-0 flex-1">
                ...
                <div className="relative grid h-[190px] grid-cols-4 items-end gap-4">
                  {[0, 1, 2, 3].map((index) => (
                    <div key={index} className="flex h-full items-end justify-center gap-1.5">
                      <div className="flex h-full flex-col items-center justify-end">
                        <span className="mb-1 text-[11px] tabular-nums text-muted-foreground">{tetanyBlocks[index]}</span>
                        <button type="button" aria-label={`${TIME_BLOCK_LABELS[index]}. Tetany ${tetanyBlocks[index]} times`} aria-pressed={active === `t${index}`}
                          ...
                          className={`w-7 rounded-t-[5px] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring ${active === `t${index}` ? "ring-2 ring-foreground/70" : ""}`}
                          style={{ height: `${Math.max(4, (tetanyBlocks[index] / max) * 100)}%`, background: TETANY_COLOR }} />
                      </div>
                      <div className="flex h-full flex-col items-center justify-end">
                        <span className="mb-1 text-[11px] tabular-nums text-muted-foreground">{panicBlocks[index]}</span>
                        <button type="button" aria-label={`${TIME_BLOCK_LABELS[index]}. Panic ${panicBlocks[index]} times`} aria-pressed={active === `p${index}`}
                          ...
                          className={`w-7 rounded-t-[5px] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring ${active === `p${index}` ? "ring-2 ring-foreground/70" : ""}`}
            ...
            <div className="mt-2 grid grid-cols-4 gap-4 pl-7 text-center text-[11px] text-muted-foreground">{TIME_BLOCK_SHORT.map((label) => <span key={label}>{label}</span>)}</div>
          </div>
