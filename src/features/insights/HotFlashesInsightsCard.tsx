...
export function HotFlashesInsightsCard({
  ...
  return (
    <section className="rounded-3xl bg-surface p-5 shadow-sm ring-1 ring-border/80">
      <p className="text-sm uppercase tracking-[0.08em] text-muted-foreground" style={{ fontWeight: 700 }}>{t("Hot flashes")}</p>
      ...
      {total ? (
        <>
          <div className="mt-4 flex flex-wrap items-baseline gap-x-2 gap-y-1">
            <span className="font-serif text-5xl leading-none text-foreground">{total}</span>
            <span className="text-base text-muted-foreground">{total === 1 ? "episode" : "episodes"} · avg {average?.toFixed(1) ?? "—"}/5 · most often L{topLevel}</span>
          </div>

          <div className="mt-4 rounded-[26px] bg-background/45 px-3 py-4 ring-1 ring-border/45">
            <p className="text-[11px] text-muted-foreground">Episodes by day</p>
            <div className="relative mt-3 grid h-[58px] items-end gap-[3px]" style={{ gridTemplateColumns: `repeat(${Math.max(1, bars.length)}, minmax(0, 1fr))` }}>
            ...
            <div className="mt-2 flex justify-between border-t border-border/50 pt-2 text-[10px] text-muted-foreground">
            ...
            <p className="mt-1 text-center text-[11px] text-muted-foreground">{period === "Y" ? "Month" : period === "M" ? "Day of month" : "Day"}</p>
            ...
            <div className="mt-4 space-y-2.5">
              {[1, 2, 3, 4, 5].map((level) => {
                ...
                const color = HOT_FLASH_COLORS[level];
                return (
                  <div key={level} className="grid grid-cols-[28px_118px_minmax(0,1fr)_28px] items-center gap-2">
                    <span className="grid h-7 w-7 place-items-center rounded-full text-xs text-white" style={{ background: color, fontWeight: 700 }}>{level}</span>
                    <span className="truncate text-sm text-muted-foreground">{HOT_FLASH_DESCRIPTIONS[level]}</span>
                    <div className="h-3 overflow-hidden rounded-full bg-tint/70"><div className="h-full rounded-full" style={{ width: `${pct}%`, background: color }} /></div>
                    <span className="text-right text-sm tabular-nums text-muted-foreground">{count}</span>
                  </div>
                );
              })}
            </div>
      ...
      ) : <p className="mt-4 text-sm text-muted-foreground">{t("No hot flashes logged")}</p>}
    </section>
  );
}
