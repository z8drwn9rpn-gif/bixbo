...
function HorizontalEmpty({ children }: { children: string }) {
  return <div className="grid min-h-[96px] place-items-center px-3 text-center text-xs text-muted-foreground">{children}</div>;
}

export function MentalInsightsCard({ data }: { data: BixboData }) {
  const { t } = useI18n();
  ...
  const topState = stateStats[0] ?? null;
  ...
  return (
    <section
      ...
      data-bixbo-distress-factor-card="true"
    >
      <div className="flex items-center gap-2.5">
        ...
        <div className="min-w-0">
          ...
          <p className="mt-0.5 text-[10px] text-muted-foreground">Distress, states and contributing factors</p>
        </div>
      </div>
      ...
      <div className="mt-3 rounded-2xl bg-background/45 px-3 pb-2.5 pt-3 ring-1 ring-border/45">
        <p className="text-[10px] font-medium text-foreground">Mental distress <span className="font-normal text-muted-foreground">(0–10)</span></p>
        <div className="mt-2 flex gap-2">
          ...
          <div className="relative min-w-0 flex-1">
            ...
            <div
              className={`relative grid h-[150px] items-end ${period === "M" ? "gap-[1px]" : "gap-[3px]"}`}
            ...
            >
              ...
              {activeDetails && active != null ? (
                <InsightFloatingTooltip
                  leftPct={((active + 0.5) / Math.max(1, buckets.length)) * 100}
      ...
      <div className="mt-3 rounded-2xl bg-background/45 px-3 py-3 ring-1 ring-border/45">
        <div className="flex items-baseline justify-between gap-2">
          <p className="text-[10px] font-medium text-foreground">Mental states</p>
          <p className="text-[9px] text-muted-foreground">frequency · colour = avg distress</p>
        </div>
        {stateStats.length ? (
          <div className="mt-3 space-y-2.5">
            {stateStats.map((item) => (
              <div key={item.raw} className="grid grid-cols-[minmax(92px,126px)_minmax(0,1fr)_24px] items-center gap-2">
                ...
                <span className="relative h-4 overflow-visible rounded-md bg-tint/45">
                  <span
                    ...
                    style={{ width: `${Math.max(5, (item.count / maxStateCount) * 100)}%`, background: vividPainChartColor(item.average) }}
                  />
                </span>
        ...
        ) : <HorizontalEmpty>No mental states logged in this period.</HorizontalEmpty>}
      </div>

      <div className="mt-3 rounded-2xl bg-background/45 px-3 py-3 ring-1 ring-border/45">
        <div className="flex items-baseline justify-between gap-2">
          <p className="text-[10px] font-medium text-foreground">Distress by factor <span className="font-normal text-muted-foreground">(avg 0–10)</span></p>
          <p className="text-[9px] text-muted-foreground">association</p>
        </div>
        {factorStats.length ? (
          <div className="mt-3 space-y-2.5">
            {factorStats.map((item) => (
              <div key={item.raw} className="grid grid-cols-[minmax(108px,138px)_minmax(0,1fr)_58px] items-center gap-2">
                <span className="truncate text-[10px] font-medium text-foreground">{item.label}</span>
                <span className="relative h-4 overflow-visible rounded-md bg-tint/45">
                  <span
                    data-bixbo-chart-mark="bar"
                    data-bixbo-chart-direction="horizontal"
                    className="absolute inset-y-0 left-0 min-w-[6px]"
                    style={{ width: `${Math.max(5, item.average * 10)}%`, background: vividPainChartColor(item.average) }}
                  />
                </span>
                <span className="text-right text-[10px] font-semibold tabular-nums text-foreground">
                  {item.average.toFixed(1)} <span className="font-normal text-muted-foreground">n={item.count}</span>
                </span>
              </div>
            ))}
          </div>
        ) : <HorizontalEmpty>No contributing factors logged in this period.</HorizontalEmpty>}
        <p className="mt-3 text-[9px] leading-snug text-muted-foreground">Factors show associations in your logs, not causes.</p>
      </div>
      ...
      <QuickInsights items={[
        ...
        {
          kind: "bars",
          color: topState ? vividPainChartColor(topState.average) : "#6ea83c",
          text: topState ? `Most common state: ${topState.label} (${topState.count} ${topState.count === 1 ? "entry" : "entries"})` : "No mental state pattern yet",
        },
        {
          kind: "moon",
          color: strongestFactor ? vividPainChartColor(strongestFactor.average) : "#6ea83c",
          text: strongestFactor ? `${strongestFactor.label} had the highest average distress (${strongestFactor.average.toFixed(1)}/10, n=${strongestFactor.count})` : "No factor pattern yet",
        },
      ]} />
