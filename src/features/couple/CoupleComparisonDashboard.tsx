  ...
  const visit = (children: ReactNode) => {
    Children.forEach(children, (child) => {
      ...
      const isComparisonMetric =
        ...
        (typeof props.mine === "number" || props.mine === null) &&
        (typeof props.theirs === "number" || props.theirs === null) &&
        typeof props.mineLabel === "string" &&
        typeof props.partnerLabel === "string" &&
      ...
      if (isComparisonMetric) {
        metrics.push({
          ...
          mineLabel: props.mineLabel,
          partnerLabel: props.partnerLabel,
          tone: props.tone as ComparisonTone,
...
function outcomeFor(metric: ComparisonMetric): Outcome {
  if (metric.mine == null || metric.theirs == null) return "similar";
  if (metric.title === "Medication doses") return "similar";
  if (metric.mine === metric.theirs) return "similar";
...
function metricPercent(metric: ComparisonMetric, value: number | null) {
  ...
  const fallbackMax = Math.max(1, Math.abs(metric.mine ?? 0), Math.abs(metric.theirs ?? 0));
...
function HighlightMetric({ label, metric }: { label: string; metric: ComparisonMetric | null }) {
  ...
  return (
    <div className="flex min-w-0 items-center gap-2 px-3 py-3">
      <span className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-background/90" style={{ color: palette.text }}>{metric.icon}</span>
      <div className="min-w-0">
        <p className="text-[10px] font-semibold" style={{ color: palette.text }}>{label}</p>
        <p className="truncate text-xs font-bold text-foreground">{metric.title}</p>
        <p className="mt-0.5 truncate text-[11px] font-semibold" style={{ color: palette.text }}>
          {formatValue(metric.mine, metric.decimals, metric.unit)} <span className="font-normal text-muted-foreground">vs</span> {formatValue(metric.theirs, metric.decimals, metric.unit)}
        </p>
      </div>
    </div>
  );
...
function FeaturedComparison({ metric }: { metric: ComparisonMetric }) {
  ...
  const difference = metric.mine != null && metric.theirs != null ? metric.mine - metric.theirs : null;
  const direction = difference == null ? "" : difference > 0 ? "higher" : difference < 0 ? "lower" : "the same";
  ...
  return (
    <article className="rounded-[1.75rem] bg-surface p-4 shadow-sm ring-1 ring-border/80">
      <div className="flex items-start gap-3">
        <span className="grid h-11 w-11 shrink-0 place-items-center rounded-full" style={{ backgroundColor: palette.soft, color: palette.text }}>{metric.icon}</span>
      ...
      <div className="mt-4 grid grid-cols-[1fr_auto_1fr] items-stretch gap-3">
        <div className="min-w-0">
          <p className="text-xs font-semibold" style={{ color: palette.text }}>{t(metric.mineLabel)}</p>
          <p className="mt-1 text-[2.4rem] font-bold leading-none tabular-nums text-foreground">{metric.mine == null ? "—" : metric.mine.toFixed(metric.decimals)}<span className="ml-1 text-sm font-medium text-muted-foreground">{metric.unit}</span></p>
          <div className="mt-2 h-2 overflow-hidden rounded-full bg-background ring-1 ring-border/40"><div className="h-full rounded-full" style={{ width: `${minePercent}%`, backgroundColor: palette.solid }} /></div>
          <p className="mt-1 text-[10px] font-medium" style={{ color: palette.text }}>{t(painDescription(metric.mine))}</p>
          <div className="mt-2"><AggregateShape percent={minePercent} color={palette.solid} /></div>
        </div>
        ...
        <div className="relative flex w-8 items-center justify-center">
          ...
          <span className="relative grid h-8 w-8 place-items-center rounded-full bg-surface text-[10px] font-semibold uppercase text-muted-foreground ring-1 ring-border">vs</span>
        </div>

        <div className="min-w-0">
          <p className="text-xs font-semibold" style={{ color: PARTNER_COLOR }}>{t(metric.partnerLabel)}</p>
          <p className="mt-1 text-[2.4rem] font-bold leading-none tabular-nums text-foreground">{metric.theirs == null ? "—" : metric.theirs.toFixed(metric.decimals)}<span className="ml-1 text-sm font-medium text-muted-foreground">{metric.unit}</span></p>
          <div className="mt-2 h-2 overflow-hidden rounded-full bg-background ring-1 ring-border/40"><div className="h-full rounded-full" style={{ width: `${theirsPercent}%`, backgroundColor: PARTNER_COLOR }} /></div>
          <p className="mt-1 text-[10px] font-medium" style={{ color: PARTNER_COLOR }}>{t(painDescription(metric.theirs))}</p>
          <div className="mt-2"><AggregateShape percent={theirsPercent} color={PARTNER_COLOR} /></div>
        </div>
      </div>
...
function MiniMetricCard({ metric }: { metric: ComparisonMetric }) {
  ...
  return (
    <article className="min-w-0 rounded-[1.4rem] bg-surface px-2.5 py-3 shadow-sm ring-1 ring-border/80">
      <span className="grid h-8 w-8 place-items-center rounded-full" style={{ backgroundColor: palette.soft, color: palette.text }}>{metric.icon}</span>
      <h4 className="mt-2 min-h-8 text-[10px] font-bold leading-tight text-foreground">{t(metric.title)}</h4>
      <p className="mt-0.5 min-h-6 text-[8px] leading-tight text-muted-foreground">{t(isMedication ? "Consistency matters" : "Lower is better")}</p>
      <div className="mt-2 flex items-baseline gap-1 text-[9px] text-muted-foreground">
        <span className="text-lg font-bold tabular-nums" style={{ color: palette.text }}>{metric.mine == null ? "—" : metric.mine.toFixed(metric.decimals)}</span>
        <span>vs</span>
        <span className="text-lg font-bold tabular-nums" style={{ color: PARTNER_COLOR }}>{metric.theirs == null ? "—" : metric.theirs.toFixed(metric.decimals)}</span>
      </div>
      <div className="mt-2 space-y-1">
        <div className="h-1.5 overflow-hidden rounded-full bg-background"><div className="h-full rounded-full" style={{ width: `${minePercent}%`, backgroundColor: palette.solid }} /></div>
        <div className="h-1.5 overflow-hidden rounded-full bg-background"><div className="h-full rounded-full" style={{ width: `${theirsPercent}%`, backgroundColor: PARTNER_COLOR }} /></div>
      </div>
    </article>
  );
}
...
export function CoupleComparisonDashboard({ children }: { children: ReactNode }) {
  ...
  const outcomes = metrics.map((metric) => ({ metric, outcome: outcomeFor(metric) }));
  ...
  const bestMetric = better.slice().sort((a, b) => comparisonStrength(b.metric) - comparisonStrength(a.metric))[0]?.metric ?? null;
  const worstMetric = worse.slice().sort((a, b) => comparisonStrength(b.metric) - comparisonStrength(a.metric))[0]?.metric ?? null;
  ...
  return (
    <section className="space-y-3">
      ...
      {miniMetrics.length ? <div className="grid grid-cols-4 gap-1.5">{miniMetrics.map((metric) => <MiniMetricCard key={metric.title} metric={metric} />)}</div> : null}

      {miniMetrics.length > 1 ? <div className="flex justify-center gap-1.5" aria-hidden="true"><span className="h-1.5 w-1.5 rounded-full bg-primary" /><span className="h-1.5 w-1.5 rounded-full bg-border" /><span className="h-1.5 w-1.5 rounded-full bg-border" /><span className="h-1.5 w-1.5 rounded-full bg-border" /></div> : null}
    </section>
  );
}
