...
export function CouplePainChart({ days, mine, theirs, partnerName, periodLabel, period }: { days: string[]; mine: Record<string, { pain?: PainEntry[] }>; theirs: Record<string, { pain?: PainEntry[] }>; partnerName: string; periodLabel: string; period: CouplePeriod }) {
  ...
  return (
    <section className="rounded-3xl bg-surface p-5 shadow-sm ring-1 ring-border/80">
      <div><h2 className="text-xs font-semibold uppercase tracking-[0.16em] text-muted-foreground">{`${t("Pain")} — ${periodLabel}`}</h2><p className="mt-1 text-xs leading-relaxed text-muted-foreground">{t(period === "Y" ? "Monthly average pain" : "Daily average pain")}. {t("Tap a bar to see its value. Solid bars are yours; striped bars belong to")} {t(partnerName)}.</p></div>
      ...
      {selectedBar ? <div className="mt-2 rounded-[1.25rem] bg-surface-sunken/40 px-2.5 py-2 text-[11px] text-foreground ring-1 ring-border/35"><button type="button" onClick={() => setSelectedBar(null)} className="flex w-full items-center justify-between gap-2 text-left" aria-label={t("Close selected pain details")}><span className="min-w-0 break-words leading-snug [overflow-wrap:anywhere]"><b>{selectedBar.owner}</b> · {selectedBar.day} · {t("Pain")} <b>{selectedBar.value.toFixed(1)}/10</b> · {t(PAIN_DESCRIPTIONS[Math.max(0, Math.min(10, Math.round(selectedBar.value)))] ?? "Pain")}</span><span className="shrink-0 text-[10px] text-muted-foreground">{t("Tap to close")}</span></button></div> : <p className="mt-2 text-center text-[10px] text-muted-foreground">{t("Tap any pain bar to see the exact value.")}</p>}
