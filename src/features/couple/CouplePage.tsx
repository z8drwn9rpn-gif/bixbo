...
import { fetchPartner, useSession } from "@/lib/cloudSync";
...
import { ComparisonBarCard, SectionCard } from "./CoupleCards";
import { CoupleOverviewPanel } from "./CoupleOverviewPanel";
import { CouplePainChart } from "./CouplePainChart";
import { PartnerHealthDashboard } from "./PartnerHealthDashboard";
import {
  average,
...
export function CouplePage({ onOpenSettings }: { onOpenSettings: () => void }) {
  ...
  const periodDisplayLabel = selectedMonth.toLocaleDateString(language === "sk" ? "sk-SK" : "en-US", {
    month: "long",
    year: "numeric",
  });
  ...
  ) => periodDays.slice().reverse().map((day) => ({
    dateKey: day,
    meds,
    medLog: medLog[day] ?? {},
  ...
  const visibleHealthDay = periodDays.includes(todayKey())
    ? todayKey()
    : (periodDays[periodDays.length - 1] ?? todayKey());

  const myFirstComparisonDay = Object.keys(view.dayLogs)
    .filter((day) => hasSymptoms(view.dayLogs[day]))
    .sort()[0] ?? null;
  const partnerFirstComparisonDay = partner
    ? Object.keys(partner.dayLogs).filter((day) => hasSymptoms(partner.dayLogs[day])).sort()[0] ?? null
    : null;
  const comparisonStartDay = myFirstComparisonDay && partnerFirstComparisonDay
  ...
  const myPainAverage = average(
    comparisonPeriodDays
      .flatMap((day) => (view.dayLogs[day]?.pain ?? []).filter((pain) => pain.entryKind !== "symptom-update"))
      .map((pain) => pain.score),
  );
  const partnerPainAverage = partner
    ? average(
      comparisonPeriodDays
        .flatMap((day) => (partner.dayLogs[day]?.pain ?? []).filter((pain) => pain.entryKind !== "symptom-update"))
        .map((pain) => pain.score),
    )
    : null;
  const myPainDays = comparisonPeriodDays.filter((day) =>
    (view.dayLogs[day]?.pain ?? []).some((pain) => pain.entryKind !== "symptom-update"),
  ).length;
  const partnerPainDays = partner
    ? comparisonPeriodDays.filter((day) =>
      (partner.dayLogs[day]?.pain ?? []).some((pain) => pain.entryKind !== "symptom-update"),
    ).length
    : 0;
  ...
  const partnerTakenDoses = partner
    ? countTakenScheduledDoses(periodDays, partner.meds ?? [], partner.medLog ?? {})
    : 0;
  ...
  const myCoupleName = view.settings.userName?.trim()
    || view.profile?.nickname?.trim()
    || view.profile?.name?.trim()
    || t("You");
  const partnerName = partner?.name || "Partner";
  const canViewPartnerCycle = ready
    && session?.user.email?.trim().toLowerCase() === "jakubikm02@gmail.com";
  const tabs: { id: CoupleTab; label: string }[] = [
  ...
  return (
    <AppShell
      title={t("Bixbo Couple")}
      stickyHeader={false}
      right={(
        <button
          type="button"
          onClick={onOpenSettings}
          aria-label={t("Couple settings")}
          className="inline-flex min-h-9 items-center justify-center rounded-full border border-border/80 bg-background/90 px-3 text-xs font-semibold text-foreground shadow-sm"
        >
          {t("Settings")}
        </button>
      )}
      ...
      <div className="flex flex-col gap-3 px-5 pb-[calc(96px+env(safe-area-inset-bottom))] pt-2 lg:grid lg:grid-cols-2 lg:items-start lg:gap-3 lg:space-y-0 lg:px-0 lg:pb-12 [&>*:first-child]:lg:col-span-2">
          ...
          >
            {tabs.map((tab) => (
              <button
                ...
                className={`min-w-0 rounded-[10px] px-2 py-1.5 text-[11px] font-semibold transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring ${activeTab === tab.id ? "bg-primary text-primary-foreground shadow-md" : "text-foreground/80 hover:bg-surface/45 hover:text-foreground"}`}
              >
                {t(tab.label)}
              </button>
            ))}
        ...
        <div style={{ order: -10 }} className="flex items-center justify-between">
          ...
          <button
            ...
            className="grid h-7 w-7 place-items-center rounded-full transition hover:bg-tint disabled:opacity-30"
          >
        ...
        {!partner ? (
          <div className="rounded-3xl bg-surface p-6 text-center ring-1 ring-border">
            ...
            <button type="button" onClick={onOpenSettings} className="mt-3 inline-block text-sm text-primary underline">
          ...
          <>
            {activeTab === "overview" ? (
              <>
                <div style={{ order: layoutOrder(view, "couple.overview", "similarity", 10) }}>
                  <CoupleOverviewPanel
                ...
                {canViewPartnerCycle && partner.gender !== "male" ? (
            ...
            {activeTab === "compare" ? (
              <>
                ...
                <div style={{ order: layoutOrder(view, "couple.compare", "healthComparison", 20) }}>
                  <SectionCard title="Health comparison" description="Solid bars are yours. Striped bars belong to your partner.">
            ...
            {activeTab === "health" ? (
              <div style={{ order: layoutOrder(view, "couple.health", "partnerDetails", 10) }}>
                <PartnerHealthDashboard
                  partnerName={t(partnerName)}
                  visibleDay={visibleHealthDay}
                  pain={partnerPain}
                  tetany={partnerTetany}
                  panic={partnerPanic}
                  medDays={partnerMeds}
                />
              </div>
