...
function BodyMetricRow({ metric }: { metric: BodyMetric }) {
  ...
  return <div className="grid grid-cols-[minmax(104px,1.5fr)_repeat(3,minmax(48px,.72fr))_42px] items-center gap-2 border-t border-border/45 px-3 py-2.5 first:border-t-0"><div className="flex min-w-0 items-center gap-2"><Ico e={metric.icon} size={25} /><div className="min-w-0"><p className="truncate text-[10px] font-semibold leading-tight text-foreground">{metric.label}</p><p className="mt-0.5 truncate text-[9px] leading-tight text-muted-foreground">{metric.detail}</p></div></div>{metric.values.map((item, index) => <MetricPhaseCell key={`${metric.label}-${item.label}`} value={item.value} max={metric.max} decimals={metric.decimals} unit={metric.unit} index={index} highlight={winner === index && item.value != null} />)}<span className="truncate text-right text-[9px] font-medium text-muted-foreground">{metric.unitLabel}</span></div>;
}
