...
function ConfidenceStar({ size = 27 }: { size?: number }) {
  return (
    <span
      ...
      style={{ fontSize: size, filter: "drop-shadow(0 2px 2px rgba(88,72,18,.2))" }}
    >
      ⭐️
    </span>
  );
...
function CombinedFrequencyIntensityCard({
  ...
  const intensityDelta = previousIntensity != null && currentIntensity != null
    ...
    <article className="rounded-2xl bg-tint/55 p-3.5 ring-1 ring-border/45">
      ...
      <div className="mt-3 grid grid-cols-2 gap-2.5">
        <div className="min-w-0 rounded-2xl bg-background/52 p-2.5 ring-1 ring-border/40">
                ...
                <div key={`${title}-${item.label}-count`} className="flex h-full min-w-0 flex-col items-center justify-end">
                  ...
                  {item.value != null ? (
                    <span
                      ...
                      style={{
                        height: `${height}%`,
                        background: item.paint,
                        opacity: item.opacity,
                        boxShadow: "inset 2px 2px 3px rgba(255,255,255,.58), inset -2px -3px 4px rgba(65,35,20,.18), 0 4px 8px rgba(45,52,35,.18)",
          ...
          <div className="mt-1 grid grid-cols-2 gap-2 text-center text-[8px] font-semibold leading-tight text-muted-foreground">
            <span className="truncate">{previousLabel}</span><span className="truncate">{currentLabel}</span>
          </div>
        </div>
          ...
          <div className="mt-4 flex min-h-[91px] flex-col justify-center">
            <div className="relative mx-2 h-12">
              ...
              {previousIntensityPct != null && currentIntensityPct != null ? (
                <span
                  className="absolute top-[22px] h-1 -translate-y-1/2 rounded-full"
                  style={{
                    left: `${Math.min(previousIntensityPct, currentIntensityPct)}%`,
                    width: `${Math.max(2, Math.abs(currentIntensityPct - previousIntensityPct))}%`,
                    background: `linear-gradient(90deg, ${PREVIOUS_COLOR}, ${color})`,
                  }}
                />
              ) : null}
              {previousIntensityPct != null ? (
                <button
                  type="button"
                  data-bixbo-chart-mark="intensity-point"
                  title={`${previousLabel}: ${previousIntensity?.toFixed(1)}${intensityUnit}`}
                  className="absolute top-[10px] h-6 w-6 -translate-x-1/2 rounded-full border-2 border-white shadow-md"
                  style={{ left: `clamp(12px, ${previousIntensityPct}%, calc(100% - 12px))`, background: PREVIOUS_COLOR }}
                  aria-label={`${previousLabel} intensity ${previousIntensity?.toFixed(1)}${intensityUnit}`}
                />
              ) : null}
              {currentIntensityPct != null ? (
                <button
                  type="button"
                  data-bixbo-chart-mark="intensity-point"
                  title={`${currentLabel}: ${currentIntensity?.toFixed(1)}${intensityUnit}`}
                  className="absolute top-[10px] h-6 w-6 -translate-x-1/2 rounded-full border-2 border-white shadow-md"
                  style={{ left: `clamp(12px, ${currentIntensityPct}%, calc(100% - 12px))`, background: color, filter: "saturate(1.55) contrast(1.08)" }}
                  aria-label={`${currentLabel} intensity ${currentIntensity?.toFixed(1)}${intensityUnit}`}
                />
              ) : null}
            </div>
            <div className="grid grid-cols-2 gap-1 text-[9px] font-bold tabular-nums">
              <div className="min-w-0 text-left" style={{ color: PREVIOUS_COLOR }}>
                <p>{previousIntensity == null ? "—" : `${previousIntensity.toFixed(1)}${intensityUnit}`}</p>
                <p className="truncate text-[8px] font-medium text-muted-foreground">{previousLabel}</p>
              </div>
              <div className="min-w-0 text-right" style={{ color }}>
                <p>{currentIntensity == null ? "—" : `${currentIntensity.toFixed(1)}${intensityUnit}`}</p>
                <p className="truncate text-[8px] font-medium text-muted-foreground">{currentLabel}</p>
              </div>
            </div>
            {intensityDelta != null ? (
              <p className={`mt-1 text-center text-[8px] font-semibold ${toneClass(intensitySummary.tone)}`}>
                {intensityDelta > 0 ? "+" : ""}{intensityDelta.toFixed(1)} vs previous
              </p>
            ) : null}
          </div>
        </div>
      ...
      <div className="mt-2.5 grid grid-cols-2 gap-2">
        <ComparisonFooter text={`Count: ${countSummary.text}`} tone={countSummary.tone} />
        <ComparisonFooter text={`Intensity: ${intensitySummary.text}`} tone={intensitySummary.tone} />
      </div>
    </article>
  );
}
  ...
  return (
    <article className="rounded-2xl bg-tint/55 p-3.5 ring-1 ring-border/45">
      ...
      <div className="mt-3 grid grid-cols-2 gap-3">
        {[
          { label: previousLabel, value: previous, paint: PREVIOUS_COLOR, opacity: 0.65 },
          { label: currentLabel, value: current, paint: color, opacity: 1 },
        ].map((item) => {
          ...
          return (
            <div key={`${title}-${item.label}`} className="min-w-0 text-center">
              ...
              <div className="relative mx-auto mt-2 flex h-20 max-w-[106px] items-end justify-center rounded-2xl bg-background/48 px-3 pb-2 ring-1 ring-border/35">
                ...
                {item.value != null ? (
                  <span data-bixbo-chart-mark="bar" title={`${item.label}: ${formatMetricValue(item.value, decimals, unit)}`} className="relative z-10 w-9 rounded-t-[8px]" style={{ height: `${Math.max(item.value === 0 ? 4 : 10, pct * 0.56)}px`, background: item.paint, opacity: item.opacity, filter: "saturate(1.5) contrast(1.06)", boxShadow: "inset 2px 2px 3px rgba(255,255,255,.5), inset -2px -2px 3px rgba(40,35,20,.14), 0 3px 6px rgba(45,52,35,.14)" }} />
    ...
    <article className="rounded-2xl bg-tint/55 p-3.5 ring-1 ring-border/45">
      ...
      <div className="mx-auto mt-2 max-w-[280px] rounded-xl bg-background/48 px-3 py-3 ring-1 ring-border/35">
        ...
        <div className="relative mt-1 h-7">
          ...
          {previousPct != null ? <span data-bixbo-chart-mark="intensity-point" title={`${previousLabel}: ${formatMetricValue(previous, decimals, unit)}`} className="absolute top-1.5 h-4 w-4 -translate-x-1/2 rounded-full" style={{ left: `clamp(8px, ${previousPct}%, calc(100% - 8px))`, background: PREVIOUS_COLOR, opacity: 0.7 }} /> : null}
          {currentPct != null ? <span data-bixbo-chart-mark="intensity-point" title={`${currentLabel}: ${formatMetricValue(current, decimals, unit)}`} className="absolute top-1 h-5 w-5 -translate-x-1/2 rounded-full" style={{ left: `clamp(10px, ${currentPct}%, calc(100% - 10px))`, background: color, filter: "saturate(1.55) contrast(1.08)", boxShadow: "0 2px 5px rgba(45,52,35,.18)" }} /> : null}
        </div>
      </div>
      <div className="mt-3"><ComparisonFooter text={summary.text} tone={summary.tone} /></div>
    </article>
...
function OverviewTile({ icon, confidence, label, value, tone }: { icon?: string; confidence?: boolean; label: string; value: string; tone: Tone }) {
  ...
  return (
    <div className={`min-w-0 rounded-2xl p-3 ring-1 ${backgrounds}`}>
      <div className="flex items-start gap-2.5">
        <span className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-background/82 shadow-sm ring-1 ring-border/55">
          {confidence ? <ConfidenceStar /> : <Ico e={icon} size={27} />}
        </span>
        <div className="min-w-0 flex-1"><p className="text-[9px] font-bold uppercase tracking-[0.06em] text-muted-foreground">{label}</p><p className={`mt-1 break-words text-[12px] font-semibold leading-snug ${toneClass(tone)}`}>{value}</p></div>
      </div>
    </div>
  );
}
  ...
  return (
    <section style={{ order }} className="overflow-hidden rounded-3xl bg-surface shadow-sm ring-1 ring-border/80 lg:col-span-2">
      <button type="button" onClick={() => setOpen((value) => !value)} aria-expanded={open} className="flex w-full items-center gap-3 px-4 py-4 text-left transition hover:bg-tint/30">
        ...
        <div className="min-w-0 flex-1"><h2 className="text-sm font-semibold text-foreground">{title}</h2><p className="mt-0.5 text-[11px] leading-relaxed text-muted-foreground">{subtitle}</p></div>
        <ChevronDown className={`h-5 w-5 shrink-0 text-muted-foreground transition-transform ${open ? "rotate-180" : ""}`} />
      </button>
      {open ? <div className="space-y-2.5 border-t border-border/45 p-3">{children}</div> : null}
    </section>
  );
}
    ...
    <>
      <section data-bixbo-jump-label={t("This month at a glance")} style={{ order: layoutOrder(view, "patterns.monthly", "overview", 10) }} className="rounded-3xl bg-surface p-4 shadow-sm ring-1 ring-border/80 lg:col-span-2">
        ...
        <div className="mt-3 grid grid-cols-2 gap-2">
          ...
          <OverviewTile icon="📝" label={t("Most stable")} value={formatChange(mostStable)} tone="neutral" />
          <OverviewTile confidence label={t("Confidence")} value={`${monthlyConfidence} · ${monthlyLoggedDays} ${t("logged days")}`} tone="neutral" />
        </div>
      </section>

      <MonthlySection title={t("Symptoms")} subtitle={t("Hot flashes, headaches and pressure")} icon="🔅" defaultOpen order={layoutOrder(view, "patterns.monthly", "symptoms", 20)}>
      ...
      <MonthlySection title={t("Lifestyle & routines")} subtitle={t("Sleep, weight, medication and workouts")} icon="🌙" defaultOpen={false} order={layoutOrder(view, "patterns.monthly", "lifestyle", 40)}>
        ...
        <VerticalComparisonCard title={t("Workouts")} subtitle={t("Number of logged workout sessions")} icon="👟" previous={workoutCurrent == null ? null : workoutPrevious.count} current={workoutCurrent.count} previousLabel={previousMonthLabel} currentLabel={currentMonthLabel} color={CHART_COLORS.workout} higherIsWorse={false} />
      ...
      <MonthlySection title={t("Hormones")} subtitle={t("PCOS and histamine changes")} icon="🌻" defaultOpen={false} order={layoutOrder(view, "patterns.monthly", "hormones", 50)}>
