...
function DashboardCard({ title, subtitle, icon, children, jumpLabel }: { title: string; subtitle: string; icon?: string; children: ReactNode; jumpLabel?: string }) {
  return (
    <section data-bixbo-jump-label={jumpLabel ?? title} className="overflow-hidden rounded-3xl bg-surface shadow-sm ring-1 ring-border/80 lg:col-span-2">
      <div className="flex items-start gap-3 px-4 pb-3 pt-4">
        {icon ? <IconChip icon={icon} /> : null}
        <div className="min-w-0 flex-1"><h2 className="text-sm font-bold uppercase tracking-[0.055em] text-foreground">{title}</h2><p className="mt-1 text-xs leading-relaxed text-muted-foreground">{subtitle}</p></div>
      </div>
  ...
  return (
    <section className="overflow-hidden rounded-3xl bg-surface shadow-sm ring-1 ring-border/80 lg:col-span-2">
      <button type="button" onClick={() => setOpen((value) => !value)} aria-expanded={open} className="flex w-full items-center gap-3 px-4 py-4 text-left transition hover:bg-tint/30">
        {icon ? <IconChip icon={icon} size={27} /> : null}
  ...
  return (
    <article className="rounded-2xl bg-tint/52 p-3.5 ring-1 ring-border/45">
      <div className="flex items-start gap-3">
        <span className="grid h-8 w-8 shrink-0 place-items-center rounded-full text-[11px] font-bold text-white" style={{ background: "linear-gradient(145deg,#a78bfa,#6d28d9)", boxShadow: "inset 1px 1px 2px rgba(255,255,255,.62), inset -1px -2px 3px rgba(45,25,80,.2), 0 2px 5px rgba(91,63,153,.28)" }}>{index + 1}</span>
        <div className="min-w-0 flex-1">
          <p className="text-sm font-semibold leading-snug text-foreground">{association.trigger} → {association.outcome}</p>
          <p className="mt-1 text-[11px] leading-relaxed text-muted-foreground">Outcome was <span className={`font-semibold ${changeToneTextClass(tone)}`}>{Math.abs(association.difference).toFixed(0)} percentage points {direction}</span> on days with this trigger.</p>
          <p className="mt-2 border-t border-border/35 pt-2 text-[10px] leading-relaxed text-muted-foreground">Based on {association.withCount} days with and {association.withoutCount} days without the trigger.</p>
        </div>
      </div>
    </article>
  );
    ...
    <>
      ...
      <DashboardCard title={t("Trigger comparison")} subtitle={t("Compare how often an outcome occurred on days with and without a possible trigger.")} icon="🎯">
        <div className="space-y-3">
          <label className="block"><span className="text-[10px] font-bold uppercase tracking-[0.08em] text-muted-foreground">{t("Possible trigger")}</span><div className="relative mt-1.5"><span className="pointer-events-none absolute left-3 top-1/2 z-10 -translate-y-1/2"><Ico e="💊" size={25} /></span><select value={selectedTrigger} onChange={(event) => setSelectedTrigger(event.target.value)} className="min-h-12 w-full appearance-none rounded-2xl bg-tint/55 py-3 pl-12 pr-11 text-sm font-medium text-foreground outline-none ring-1 ring-border/55 focus:ring-2 focus:ring-primary">{triggerOptions.map((option) => <option key={option.id} value={option.id}>{option.label}</option>)}</select><ChevronDown className="pointer-events-none absolute right-4 top-1/2 h-5 w-5 -translate-y-1/2 text-primary" /></div></label>
          <label className="block"><span className="text-[10px] font-bold uppercase tracking-[0.08em] text-muted-foreground">{t("Compare with outcome")}</span><div className="relative mt-1.5"><span className="pointer-events-none absolute left-3 top-1/2 z-10 -translate-y-1/2"><Ico e={iconForAssociation(selectedOutcomeLabel)} size={25} /></span><select value={selectedOutcome} onChange={(event) => setSelectedOutcome(event.target.value)} className="min-h-12 w-full appearance-none rounded-2xl bg-tint/55 py-3 pl-12 pr-11 text-sm font-medium text-foreground outline-none ring-1 ring-border/55 focus:ring-2 focus:ring-primary">{outcomeOptions.map((option) => <option key={option.id} value={option.id}>{option.label}</option>)}</select><ChevronDown className="pointer-events-none absolute right-4 top-1/2 h-5 w-5 -translate-y-1/2 text-primary" /></div></label>
        </div>
