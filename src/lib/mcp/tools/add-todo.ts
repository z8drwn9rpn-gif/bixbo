...
export default defineTool({
  name: "add_todo",
  title: "Add todo",
  description: "Add a to-do item to a BIXBO calendar day.",
  inputSchema: {
  ...
  handler: async ({ text, date }, ctx) => {
    ...
    const todos = { ...(blob.todos ?? {}) };
    const item = { id: newId(), text, done: false };
    todos[day] = [...(todos[day] ?? []), item];
    await saveBlob(ctx, { ...blob, todos });
    return json({ date: day, added: item, total: todos[day].length });
  },
});
