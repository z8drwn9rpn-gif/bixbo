...
describe("iPhone PWA launch presentation", () => {
  it("shows the BIXBO penguin for one painted second before handing off to the app", () => {
    ...
    expect(root).toContain('window.addEventListener("pageshow", holdAfterFirstVisiblePaint, { once: true })');
    expect(root).toContain('splashImage.addEventListener("load", beginVisibleSecond, { once: true })');
    ...
    expect(root).not.toContain('window.addEventListener("load", showForOneSecond');
