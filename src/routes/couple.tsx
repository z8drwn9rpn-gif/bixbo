...
import { BlueberrySection } from "@/features/couple/BlueberrySection";
import { CouplePainChart } from "@/features/couple/CouplePainChart";
import {
  ComparisonBarCard,
  CurrentAndHistory,
  MedsList,
  ...
  StatCard,
  TetanyList,
} from "@/features/couple/CoupleCards";
import {
  average,
  countTakenScheduledDoses,
  coupleRangeFor,
  hasSymptoms,
  isSameMonth,
  startOfMonth,
  ...
  type CouplePeriod,
} from "@/features/couple/coupleUtils";

export const Route = createFileRoute("/couple")({
  head: () => ({
    meta: [
      { title: "Bixbo Couple" },
      { name: "description", content: "Share and compare pain, tetany, panic and medication patterns with your partner." },
      { property: "og:title", content: "Bixbo Couple" },
      { property: "og:description", content: "Private partner sharing for selected health categories." },
    ],
  }),
...
function CouplePage() {
  ...
  useEffect(() => {
    ...
    const intervalId = window.setInterval(() => {
      if (document.visibilityState === "visible") void refreshPartnerView();
    }, 4000);
  ...
  const periodDisplayLabel = period === "M"
    ? selectedMonth.toLocaleDateString(language === "sk" ? "sk-SK" : "en-US", { month: "long", year: "numeric" })
    : range.label;
  ...
  const goPrev = () => setAnchor((current) => {
    const next = new Date(current);
    next.setDate(1);
    next.setMonth(next.getMonth() - 1);
    return next;
  });
  const goNext = () => setAnchor((current) => {
    const next = new Date(current);
    next.setDate(1);
    next.setMonth(next.getMonth() + 1);
  ...
  const collectPain = (dayLogs: Record<string, ComparableDayLog>) => {
    ...
    for (const day of periodDays) {
      for (const pain of dayLogs[day]?.pain ?? []) output.push({ ...pain, dateKey: day });
    }
  ...
  const collectTetany = (dayLogs: Record<string, ComparableDayLog>) => {
    ...
    for (const day of periodDays) {
      for (const episode of dayLogs[day]?.tetany ?? []) output.push({ ...episode, dateKey: day });
    }
  ...
  const collectPanic = (dayLogs: Record<string, ComparableDayLog>) => {
    ...
    for (const day of periodDays) {
      for (const attack of dayLogs[day]?.panic ?? []) output.push({ ...attack, dateKey: day });
    }
    return output.sort((a, b) => b.dateKey.localeCompare(a.dateKey)).slice(0, 20);
  };
  ...
  const splitEntriesByVisibleDay = <T extends { dateKey: string }>(entries: T[]) => ({ current: entries.filter((entry) => entry.dateKey === visibleHealthDay), history: entries.filter((entry) => entry.dateKey !== visibleHealthDay) });
  const splitMedDaysByVisibleDay = (days: { dateKey: string; meds: Med[]; medLog: Record<string, boolean>; extra: ExtraMed[] }[]) => ({ current: days.filter((day) => day.dateKey === visibleHealthDay), history: days.filter((day) => day.dateKey !== visibleHealthDay) });
  ...
  const tabs: { id: CoupleTab; label: string; icon: ReactNode }[] = [
    { id: "overview", label: "Overview", icon: <HeartIcon size={24} /> },
    { id: "compare", label: "Compare", icon: <SparkleIcon size={24} /> },
    { id: "health", label: "Health", icon: <LeafIcon size={24} /> },
  ];
  ...
  return (
    <AppShell title={t("Bixbo Couple")}>
      <div className="flex flex-col gap-3 px-5 pb-[calc(96px+env(safe-area-inset-bottom))] pt-2 lg:grid lg:grid-cols-2 lg:items-start lg:gap-3 lg:space-y-0 lg:px-0 lg:pb-12 [&>*:first-child]:lg:col-span-2">
        ...
        {partner ? <nav style={{ order: -20 }} aria-label={t("Couple sections")} className="mx-auto grid w-full max-w-[340px] grid-cols-3 gap-0.5 rounded-xl bg-primary/20 p-0.5 ring-1 ring-primary/15 lg:max-w-sm">{tabs.map((tab) => { const active = activeTab === tab.id; return <button key={tab.id} type="button" onClick={() => setActiveTab(tab.id)} aria-pressed={active} className={`min-w-0 rounded-[10px] px-2 py-1.5 text-[11px] font-semibold transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring ${active ? "bg-primary text-primary-foreground shadow-md" : "text-foreground/80 hover:bg-surface/45 hover:text-foreground"}`}>{t(tab.label)}</button>; })}</nav> : null}
        <div style={{ order: -10 }} className="flex items-center justify-between"><button type="button" onClick={goPrev} aria-label={t("Previous period")} className="grid h-7 w-7 place-items-center rounded-full transition hover:bg-tint focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"><ChevronLeft className="h-3 w-3" /></button><span className="text-[11px] font-medium leading-none">{periodDisplayLabel}</span><button type="button" onClick={goNext} disabled={!canGoNext} aria-label={t("Next period")} className="grid h-7 w-7 place-items-center rounded-full transition hover:bg-tint focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring disabled:cursor-not-allowed disabled:opacity-30"><ChevronRight className="h-3 w-3" /></button></div>

        {!partner ? <div className="rounded-3xl bg-surface p-6 text-center ring-1 ring-border"><p className="text-sm font-medium">{t("No partner linked yet.")}</p><p className="mt-1 text-xs leading-relaxed text-muted-foreground">{t("In Settings → Couple sharing, exchange pairing codes with your partner to compare selected health logs.")}</p><Link to="/settings" className="mt-3 inline-block text-sm text-primary underline">{t("Open Couple sharing")}</Link></div> : <>
