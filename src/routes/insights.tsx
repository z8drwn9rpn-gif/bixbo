...
function InsightsPage() {
  ...
  const cardClass = "rounded-3xl border border-border/70 bg-surface p-4 shadow-sm ring-1 ring-border/70 sm:p-5";

  return <AppShell title={t("Health of Bixbo")}>
    <div className="sticky top-[var(--bixbo-header-offset,3.5rem)] z-20 border-b border-border/45 bg-background/92 px-5 py-2 backdrop-blur-xl supports-[backdrop-filter]:bg-background/84 lg:static lg:border-0 lg:bg-transparent lg:px-0 lg:backdrop-blur-none">
      <div className="grid grid-cols-2 rounded-2xl bg-tint p-1 ring-1 ring-border/70 lg:mx-auto lg:w-full lg:max-w-[420px]">
        {(["insights", "patterns"] as const).map((value) => <button key={value} type="button" onClick={() => setOverviewView(value)} className={`min-h-10 rounded-xl px-4 py-2 text-center text-sm font-semibold transition ${overviewView === value ? "bg-primary text-primary-foreground shadow-sm" : "text-muted-foreground hover:bg-surface/70 hover:text-foreground"}`} aria-pressed={overviewView === value}>{value === "insights" ? t("Insights") : t("Patterns")}</button>)}
      </div>

      {overviewView === "insights" ? <div className="mt-2 -mx-1 flex snap-x gap-1.5 overflow-x-auto px-1 pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden" role="group" aria-label={t("Insights sections")}>
        {([["all", "All"], ["overview", "Overview"], ["pain", "Pain"], ["symptoms", "Symptoms"], ["bowel", "Bowel"], ["meds", "Meds"]] as const).map(([id, label]) => <button key={id} type="button" onClick={() => setInsightsFilter(id)} aria-pressed={insightsFilter === id} className={`min-h-9 shrink-0 snap-start rounded-full px-3.5 py-1.5 text-[11px] font-semibold transition ${insightsFilter === id ? "bg-primary text-primary-foreground shadow-sm" : "bg-surface text-muted-foreground ring-1 ring-border/70 hover:text-foreground"}`}>{t(label)}</button>)}
      </div> : null}
    </div>

    {overviewView === "patterns" ? <PatternsContent /> : <div className="flex flex-col gap-3 px-5 pt-3 pb-[calc(96px+env(safe-area-inset-bottom))] lg:grid lg:grid-cols-2 lg:items-start lg:px-0 lg:pb-12">
      <div className={insightsFilter === "all" || insightsFilter === "overview" ? "min-w-0 lg:col-span-2" : "hidden"} style={{ order: layoutOrder(view, "insights", "heatmap", 10) }}><YearHealthHeatmap data={view} anchor={anchor} onShiftPeriod={shiftHeatmapPeriod} /></div>

      <section style={{ order: layoutOrder(view, "insights", "pain", 20) }} className={`${insightsFilter === "all" || insightsFilter === "pain" ? "" : "hidden "}${cardClass} min-w-0 overflow-hidden`}>
        ...
        <div className="mt-2 flex flex-wrap items-baseline gap-x-2 gap-y-1"><span className="font-serif text-4xl leading-none">{painAvg != null ? painAvg.toFixed(1) : "–"}</span><span className="text-sm text-muted-foreground">avg · {painSeries.filter((value) => value != null).length} {painSeries.filter((value) => value != null).length === 1 ? "entry" : "entries"}</span></div>
        <PainChart key={`pain-${painPeriod}-${toKey(painAnchor)}`} period={painPeriod} days={painDays} series={painSeries} anchor={painAnchor} />
      </section>

      <section style={{ order: layoutOrder(view, "insights", "hotFlashes", 30) }} className={`${insightsFilter === "all" || insightsFilter === "symptoms" ? "" : "hidden "}${cardClass} min-w-0 overflow-hidden`}>
        ...
        {hfTotal ? <><div className="mt-2 flex flex-wrap items-baseline gap-x-2 gap-y-1"><span className="font-serif text-4xl leading-none">{hfTotal}</span><span className="text-sm text-muted-foreground">{hfTotal === 1 ? "episode" : "episodes"} · avg {hfAvg!.toFixed(1)}/5 · most often L{hfTop}</span></div><HfBars key={`hot-flashes-${hotFlashPeriod}-${toKey(hotFlashAnchor)}`} bars={hfBars} period={hotFlashPeriod} days={hotFlashDays} anchor={hotFlashAnchor} /><div className="mt-3 space-y-1.5">{[1, 2, 3, 4, 5].map((level) => { const count = hfCounts[level]; const pct = hfTotal ? (count / hfTotal) * 100 : 0; const color = HOT_FLASH_COLORS[level]; return <div key={level} className="flex min-w-0 items-center gap-2 text-[10px]"><span className="grid h-4 w-4 shrink-0 place-items-center rounded-full text-[10px] font-bold text-white" style={{ background: color }}>{level}</span><span className="w-16 shrink-0 text-muted-foreground">{HOT_FLASH_DESCRIPTIONS[level]}</span><div className="h-2 min-w-0 flex-1 overflow-hidden rounded-full bg-tint"><div className="h-full rounded-full" style={{ width: `${pct}%`, background: color }} /></div><span className="w-6 shrink-0 text-right tabular-nums text-muted-foreground">{count}</span></div>; })}</div></> : <p className="mt-2 text-sm text-muted-foreground">{t("No hot flashes logged")}</p>}
      </section>

      <div className={insightsFilter === "all" || insightsFilter === "bowel" ? "min-w-0" : "hidden"} style={{ order: layoutOrder(view, "insights", "bowel", 40) }}><BristolChart bowelCounts={bowelCounts} period={bowelPeriod} anchor={bowelAnchor} onPeriodChange={setBowelPeriod} onPeriodShift={(delta) => setBowelAnchor((current) => shiftInsightPeriodAnchor(current, bowelPeriod, delta))} /></div>
      <div className={insightsFilter === "all" || insightsFilter === "symptoms" ? "min-w-0" : "hidden"} style={{ order: layoutOrder(view, "insights", "timeOfDay", 50) }}><TimeOfDayPatternChart data={view} days={timeOfDayDays} period={timeOfDayPeriod} anchor={timeOfDayAnchor} onPeriodChange={setTimeOfDayPeriod} onPeriodShift={(delta) => setTimeOfDayAnchor((current) => shiftInsightPeriodAnchor(current, timeOfDayPeriod, delta))} /></div>
      <div className={insightsFilter === "all" || insightsFilter === "meds" ? "min-w-0" : "hidden"} style={{ order: layoutOrder(view, "insights", "meds", 60) }}><MedsAdherence data={view} period={medsPeriod} anchor={medsAnchor} onPeriodChange={setMedsPeriod} onPeriodShift={(delta) => setMedsAnchor((current) => shiftInsightPeriodAnchor(current, medsPeriod, delta))} /></div>
    </div>}
  </AppShell>;
}
