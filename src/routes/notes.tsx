...
function NotesPage() {
  ...
  const collectionScreen = screen === "recipes" || screen === "coffee";
  ...
  return (
    <AppShell
      title={title}
      right={collectionScreen && !openFolder ? undefined : (
        <button
          type="button"
          onClick={() => createNote()}
          ...
          <>
            <div className="mx-auto grid w-full max-w-[620px] grid-cols-3 gap-1 rounded-full bg-tint p-1 ring-1 ring-border/60">
              {[
                ...
                const active = item.key === "all" ? !collectionScreen : screen === item.key;
                return (
                  <button
                    key={item.key}
                    type="button"
